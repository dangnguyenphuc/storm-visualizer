...
def find_objects(search_box, image2):
    ...
    if not search_box['valid']:
        ...
        search_area = image2[search_box['z1']:search_box['z2'],
...
def shifts_disagree(shift1, shift2, record, params):
    ...
    parameter. """
    shift1 = shift1*record.grid_size
    shift2 = shift2*record.grid_size
...
def clip_shift(shift, record, params):
    ...
    shift_meters = shift * record.grid_size
    ...
    if velocity > params['MAX_FLOW_MAG']:
        ...
        clipped_pix = clipped/record.grid_size
        return clipped_pix
    else:
        return shift
...
def predict_search_extent(obj1_extent, shift, params, grid_size):
    ...
    search_radius_z = params['SEARCH_MARGIN'] / grid_size[0]
    search_radius_y = params['SEARCH_MARGIN'] / grid_size[1]
    search_radius_x = params['SEARCH_MARGIN'] / grid_size[2]
    z1 = shifted_center[0] - search_radius_z
    z2 = shifted_center[0] + search_radius_z + 1
    y1 = shifted_center[1] - search_radius_y
    y2 = shifted_center[1] + search_radius_y + 1
    x1 = shifted_center[2] - search_radius_x
    x2 = shifted_center[2] + search_radius_x + 1
    x1 = int(x1)
    x2 = int(x2)
    y1 = int(y1)
    y2 = int(y2)
...
def check_search_box(search_box, img_dims):
    ...
    if search_box['z2'] > img_dims[0]:
        search_box['z2'] = img_dims[0]
    if search_box['y2'] > img_dims[1]:
        search_box['y2'] = img_dims[1]
    if search_box['x2'] > img_dims[2]:
        search_box['x2'] = img_dims[2]
    if ((search_box['z2'] - search_box['z1'] < 5) or
        (search_box['y2'] - search_box['y1'] < 5) or 
...
def get_disparity(obj_found, image2, search_box, obj1_extent):
    ...
    for target_obj in obj_found:
        ...
        size_changed = get_sizeChange(target_extent['obj_volume'],
                                      obj1_extent['obj_volume'])
